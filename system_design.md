...
A Redis-only lock introduces the risk of split-brain scenarios or race conditions if the lock expires before the database transaction fully commits. By using `ShowSeat.objects.select_for_update().get(...)`, the platform delegates locking to the ACID-compliant relational database. 
...
Application-level locks (`select_for_update()`) are powerful, but they only protect code paths that explicitly use them. If a rogue Celery task, manual Django Admin override, or raw SQL query bypasses the ORM, data integrity could be compromised.
...
Originally, the platform used two asynchronous Celery Beat tasks running every 30 seconds:
